<?php 
  ...
  //男女別平均年齢を取得
  $aveage[] = array();
  $sql = 'SELECT `gender`,TRUNCATE(AVG(`age`))AS aveage FROM `friends` WHERE `area_id`=$area_id GROUP BY `gender`';
  $stmt = $dbh -> prepare($sql);
  $stmt -> execute();
  $rec = $stmt->fetch(PDO::FETCH_ASSOC);
  $aveage = $rec;
  ...
  if (isset($_GET['action'])&&!empty($_GET['action'])) {
    if ($_GET['action'] == 'delete') {
        $sql = 'DELETE FROM `friends` WHERE `friend_id`='.$_GET['id'];
        $stmt = $dbh->prepare($sql);
        $stmt ->execute();
...
<html lang="ja">
  <head>
    ...
    function destroy(friend_id){
        if (confirm('削除しますか')){
        //okbボタンを押した時
        location.href = 'show.php?action=delete&id=' + friend_id;
            return true;
        }else{
            return false;
        }
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="well">
        男性：<?php echo $male; ?>名　女性：<?php echo $female; ?>名</a></br>
        男性年齢:<?php echo $aveage[0]['aveage']; ?>歳　女性 :<?php echo $aveage[1]['aveage']; ?>
      </div>
            ...
            <tr>
              ...
              <td>
                <div class="text-center">
                  ...
                  <a href="#" onclick="destroy(<?php echo $fri['friend_id']; ?>);"><i class="fa fa-trash"></i></a>
                </div>
              </td>
            </tr>
            <?php } ?>
